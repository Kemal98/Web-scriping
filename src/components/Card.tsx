  ...
  return (
    <>
      ...
      <a className='relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl' href='#'>
        <img className='peer absolute top-0 right-0 h-full w-full object-cover'src={pro.product.image}
      ...
      <div className='mt-4 px-5 pb-5'>
        <a href='#'>
          <h5 className='text-xl tracking-tight text-slate-900'>{pro.product.title}</h5>
          <h5 className='text-xl tracking-tight text-slate-500'>{pro.product.category}</h5>

        </a>
        <div className='mt-2 mb-5 flex items-center justify-between'>
          <p>
            <span className='text-3xl font-bold text-slate-900'>{pro.product?.currentPrice}</span>
            <span className='text-sm text-slate-900 line-through'>{pro.product?.currency}</span>
          </p>
        </div>
      </div>
    </div>
